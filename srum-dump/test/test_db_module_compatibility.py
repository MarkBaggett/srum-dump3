...
class TestSRUMDatabaseCompatibility(unittest.TestCase):
    ...
    def test_get_records(self):
        ...
        for table_name in self.dissect_db.get_tables():

            if table_name in self.dissect_db.skip_tables:
                continue
            ...
            if len(dissect_records) != len(pyesedb_records):
                print(f"Warning table: {table_name} Record counts differ")
    ...
    def test_record_value_types(self):
        ...
        table_name = list(self.dissect_db.get_tables())[8]
